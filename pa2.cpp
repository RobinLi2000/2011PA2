...
int findSubLenAtStrPosWithQmark(const char str[], const char pattern[], int strPos = 0, int patPos = 0)
{
    ...
    if (pattern[patPos]!= NULL_CHAR && str[strPos] == NULL_CHAR && pattern[patPos] != QMARK) //pattern太长了
        return NOT_FOUND;
    ...
    if (pattern[patPos] == QMARK && str[strPos] != NULL_CHAR) //pattern此位是 “?”
    {
...
int findSubLenAtStrPosWithPercent(const char str[], const char pattern[], int strPos = 0, int patPos = 0)
{
    ...
    if (pattern[patPos] != NULL_CHAR && str[strPos] == NULL_CHAR && pattern[patPos] != PERCENT ) //pattern太长了
        return NOT_FOUND;

    if (pattern[patPos] == NULL_CHAR && str[strPos] == NULL_CHAR)
        return 0;

    if (pattern[patPos] == PERCENT)  //pattern此位是 “%”
    {
